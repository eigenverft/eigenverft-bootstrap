...
$targetConfigAllowedLicenses = Join-Segments -Segments @($topLevelDirectory, "workflows" , ".config", "allowed-licenses.json")
$targetConfigLicensesMappings = Join-Segments -Segments @($topLevelDirectory,"workflows" , ".config", "licenses-mapping.json")
...
foreach ($projectFile in $solutionProjectsObj) {
    ...
    if ($isPackable -eq $true)
    {
        ...
        Replace-FilePlaceholders -InputFile "$topLevelDirectory/workflows/.config/docfx/build/docfx_local_template.json" -OutputFile "$topLevelDirectory/.config/docfx/build/docfx_local.json" -Replacements $replacements
        dotnet docfx "$topLevelDirectory/workflows/.config/docfx/build/docfx_local.json"
    }
